...
lex_string (char *code, int *i) {
    ...
    while (code[*i] != '\"') {
        ...
        out->lexeme =
            realloc (out->lexeme, strlen (out->lexeme) + strlen (cc) + 1);
...
static inline Token *
lex_identifier (char *code, int *i) {
    Token *out = malloc (sizeof (Token));
    ...
    else if (compare (out->lexeme, "bool") || compare (out->lexeme, "boolean")
             || compare (out->lexeme, "boole")
             || compare (out->lexeme, "Boole")) {
    ...
    else if (compare (out->lexeme, "mut") || compare (out->lexeme, "mutable")) {
        out->type = TOKEN_MUTABLE;
    }
...
void
init_token_list (TokenList **listptr) {
...
void
free_token_list (TokenList *list) {
    free (list->array);
    free (list);
}
...
void
append_token (TokenList *list, Token *element) {
    if (!element)
        return;
    ...
    if (list->size == list->capacity) {
        list->capacity *= 2;
        list->array =
            realloc (list->array, list->capacity * sizeof (Token *));
    }
...
void
walk_token_list (TokenList *list, TokenWalkFn fn) {
...
static inline Token *
comment_stuff (char *code, int *i, TokenList *list) {
    if (code[(*i) + 1] == '/') {
        while (code[*i] != '\n' && code[*i] != EOF) {
            (*i)++;
        }
...
static inline void
analyze (char *code, int *i, TokenList *list) {
    Token *token;
